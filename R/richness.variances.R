...
richness.variances <- function(my.files, Boxplot=TRUE){
  ...
  richn$richness_var_archs <- richness_var_archs
  if(Boxplot==TRUE){
    richn$Boxplot <- graphics::boxplot(richness_var_archs[-which(names(richness_var_archs)=="NA")], xlab="Archetypes", ylab="Richness variance")
  }
  return(richn)
  # rich <- list()
